...
Metacells construction using SEACells requires 3 main inputs: i) an anndata object (`build_kernel_on` parameter), 
ii) a key indicating which matrix in the `obsm` attribute of the anndata object should be considered to compute the kernel needed for archetypal analysis (`build_kernel_on` parameter), and 
iii) the number of metacells to identify (`n_SEACells` parameter), which is used as inpput of the archetypal analysis.
...
In this tutorial, we will use in the SEACells model the 10 first principal components resulting from the PCA.
...
In this example, we choose a graining level of 75 as recommended by the SEACells authors.  
