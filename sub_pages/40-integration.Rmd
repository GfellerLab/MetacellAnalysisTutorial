...
Sys.setenv(RETICULATE_PYTHON = use_python("/Users/admin/miniconda3/bin/python"))
```
...
```{bash, eval = FALSE, results = FALSE, message=FALSE, warning=FALSE}
#git clone https://github.com/GfellerLab/MetacellAnalysisToolkit
MCAT_path=/Users/admin/Documents/GitHub/MetacellAnalysisToolkit/
start=`date +%s`
for d in /Users/admin/Documents/GitHub/MetacellAnalysisTutorial/data/HLCA/datasets/*;
...
library(anndata)
adata <- read_h5ad(paste0(root_dir,"/data/HLCA/local.h5ad"),backed = "r")
...
```{r read_metacells_obj, results = FALSE, message=FALSE, warning=FALSE}

metacell.files <- sapply(datasets, FUN = function(x){paste0(root_dir,"/data/HLCA/datasets/",x,"/mc_adata.h5ad")})
...
```{r unintegrated_vlnPlot_obj, results = FALSE, message=FALSE, warning=FALSE}
...
```{r unintegrated_boxplot_obj, results = FALSE, message=FALSE, warning=FALSE}
...
```{r unintegrated_preprocess, results = FALSE, message=FALSE, warning=FALSE}
...
```{r integrate, results = FALSE, message=FALSE, warning=FALSE}
...
```{r integrated_umap, fig.height=10, fig.width=14}
...
```{r integrated_dimplots, results = FALSE, message=FALSE, warning=FALSE}
library(ggplot2)
...
```{r integrated_clustering, results = FALSE, message=FALSE, warning=FALSE}
DefaultAssay(combined.mc) <- "integrated"
...
```{r integrated_findmarkers}
DefaultAssay(combined.mc) <- "RNA"
combined.mc[["RNA"]] <- JoinLayers(combined.mc[["RNA"]])
...
```{r integrated_celltype_abondance, results = FALSE, message=FALSE, warning=FALSE}
library(reshape2)
...
```{r combined_data_save}
saveRDS(combined.mc,paste0(root_dir,"/data/HLCA/combined.mc.unsup.rds"))
```
