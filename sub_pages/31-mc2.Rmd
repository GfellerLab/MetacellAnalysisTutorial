...
```{r, include=FALSE}
# This code is not ready yet, so we do not run the chunks 
TO_RUN_CHUNKS <- FALSE
```
...
#### Load data {-}
```{r load-data, child='./functional_chunks/load_anndata.Rmd', eval = TO_RUN_CHUNKS}
```


**Setup MC object**
```{python, eval = TO_RUN_CHUNKS}
...
```{python mc2-gene-filtering, collapse = TRUE, result = 'hold', eval = TO_RUN_CHUNKS}
...
```{python mc2-cell-filtering-first, warning = FALSE, collapse = TRUE, result = 'hold', eval = TO_RUN_CHUNKS}
...
```{r, child='./functional_chunks/mc2_cell_filtering_first.Rmd', result = 'asis', eval = TO_RUN_CHUNKS}
```


```{python mc2-cell-filtering-second, collapse = TRUE, result = 'hold', eval = TO_RUN_CHUNKS}
## The second round (content of non-clean genes, e.g., mito-genes)
properly_sampled_max_excluded_genes_fraction = 0.25
```

```{r mc-cell-filtering-run-second-knitr, child='./functional_chunks/mc2_cell_filtering_second.Rmd', eval = TO_RUN_CHUNKS}

```


```{r mc-extract-clean-cell-knitr, child='./functional_chunks/mc2_extract_clean_data.Rmd', eval = TO_RUN_CHUNKS}
...
```{python estimate-target-mc-size, eval = TO_RUN_CHUNKS}
...
```{python mc2-divide-n-conquer, message=FALSE, warning=FALSE, result=FALSE, eval = TO_RUN_CHUNKS}
...
```{r mc2-gamma-deviation, child='./functional_chunks/mc2_gamma_deviation.Rmd', eval = TO_RUN_CHUNKS}
...
#### Visualize metacells {-}
```{python mc2-comp-umap, result = FALSE, eval = TO_RUN_CHUNKS}
...
```{python mc2-plot-umap, eval = TO_RUN_CHUNKS}
plt.figure()
sns.scatterplot(x=umap_x, y=umap_y)
plt.show()
```

```{python mc2-membership, result = FALSE, eval = TO_RUN_CHUNKS}
...
```{r mc2-letent-space, child = './functional_chunks/mc2_latent_space.Rmd', eval = TO_RUN_CHUNKS}
```


### Metacell QC {-}
```{r mc-qc, chuld = './functional_chunks/MC_QC.Rmd', eval = TO_RUN_CHUNKS}
```
