...
sc_data = readRDS(paste0(root_dir, "data/", proj_name, "/singlecell_seurat_filtered.rds"))
```
...
MC <- SuperCell::SCimplify(Seurat::GetAssayData(sc_data, layer = "data"),  # single-cell log-normalized gene expression data
                           k.knn = k_knn,
                           gamma = gamma,
...
MC.GE <- supercell_GE(Seurat::GetAssayData(sc_data, layer = "counts"),
                      MC$membership,
                      mode =  "sum"
                      )
dim(MC.GE) 
...
PCA.res <- irlba::irlba(scale(Matrix::t(GetAssayData(sc_data,assay = "RNA",layer = "data")[MC$genes.use, ])), nv = nb_pc)
pca.x <- PCA.res$u %*% diag(PCA.res$d)
rownames(pca.x) <- colnames(GetAssayData(sc_data,assay = "RNA",layer = "data"))
...
# setwd("/Users/admin/Documents/GitHub/MetacellAnalysisTutorial/")
print(paste0("Saving metacell object for the ", proj_name, " dataset using ", MC_tool))
saveRDS(MC.seurat, file = paste0(root_dir, 'data/', proj_name, '/metacell_', MC_tool,'.rds'))
```
...
MC.seurat.ad <- anndata::AnnData(
  X = Matrix::t(Seurat::GetAssayData(MC.seurat, layer = "counts")),
  obs = MC.seurat@meta.data
)

# setwd("/Users/admin/Documents/GitHub/MetacellAnalysisTutorial/")
anndata::write_h5ad(anndata = MC.seurat.ad, filename = paste0(root_dir, 'data/', proj_name, '/metacell_', MC_tool,'.h5ad'))
```
